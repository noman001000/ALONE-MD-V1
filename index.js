    ...
    zk.ev.on("call", async callData => {
      if (conf.ANTICALL === 'yes') {
        ...
        await zk.sendMessage(callerId, {
          text: "```❗📵I AM ALONE MD | I REJECT THIS CALL BECAUSE MY OWNER IS BUSY.KINDLY SEND TEXT INSTEAD``` ."
        });
      }
    });
...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      const superUserNumbers = [servBot, "255673750170", '255673750170', '255673750170', "255673750170", '255673750170', conf.NUMERO_OWNER].map(s => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
      const allAllowedNumbers = superUserNumbers.concat(sudo);
      const superUser = allAllowedNumbers.includes(auteurMessage);
      var dev = ['255673750170', '255673750170', "255673750170", '255673750170'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
...
╰═════════════════⊷ `
        ...
        if (raisonDeconnexion === baileys_1.DisconnectReason.badSession) {
          ...
          console.log('connection error😞 ,,Alone trying to reconnect... ');
          main();
